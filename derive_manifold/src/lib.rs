        ...
        if let syn::Data::Enum(DataEnum {
            ...
            quote! {
                impl #impl_generics Manifold for #typename #ty_generics #where_clause {
                    fn connect(ds: Store) -> ::futures::stream::SelectAll<Pin<Box<dyn ::futures::Stream<Item = Result<(Vec<u8>, Self), anyhow::Error>> + Send>>> {
        ...
        self.variants.iter().flat_map(|v| match v.0.fields {
            Fields::Unnamed(ref un) => {
                un.unnamed.first().map(|_| {
                    ...
                    quote! {
                        Pipe::from_source(Source(ds.0.watch_prefix(b"foo/")), b"foo/").map(|res| res.map(|(k, v)| (k, #typename::#name(v)))).boxed(),
                    }
                })
            },
            _ => None
